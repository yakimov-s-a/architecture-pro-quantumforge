...
async def chat(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ok:
        await update.message.reply_text(f"Prompt injection detected in the user question with a score {score:.2f}.")
        return
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        if not ok:
            await update.message.reply_text(f"Prompt injection detected in context with score {score:.2f}.")
            return
